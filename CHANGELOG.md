...
## v3.70 — One place where a speed is declared

**Why.** Every rate and speed in this app was a bare number in a parameter block, and the
conversion between ticks, hours, seconds and metres was written out by hand at every site that
needed one. Three models each did it their own way, and nothing compared them.

**What.** `units.js` (`WT.units`): a quantity carries a value, a unit and a provenance; an exact
conversion arithmetic between units per hour, units per tick, seconds and ticks per unit, metres per
second, metres per minute and cells per tick, refused across dimensions by name rather than silently
scaled; and a REGISTRY of all **38** rates and speeds in the app with the owner of each and whether
it is cited, a teaching value, or measured on a named source.

**Listing them together is what this release is for.** Five disagreements became visible, and the
release states every one of them instead of quietly reconciling it, because reconciling changes
behaviour:

1. **One tick is not one tick.** `flowsim.js` declares 60 ticks per hour, so a tick is a minute.
   `wms.js` declares 4, so a tick is a quarter of an hour. Neither model reads the other's constant.
2. **The two travel speeds differ by a factor of 205.71.** The animation moves a unit at 0.35 cells
   per tick, which with a one-metre cell and a one-minute tick is 0.35 m/min, about 0.0058 m/s. The
   pick-travel model walks its picker at 1.2 m/s, which is 72 m/min. Nothing had ever converted the
   two into the same unit, which is why nobody had noticed.
3. **A conveyor has no speed.** It is geometry that reshapes a path, plus a dimensionless count
   multiplier capped at 2.2, plus a units-per-hour figure with no length. No belt length and band
   speed anywhere become a transit time.
4. **A declared speed nothing reads.** `library.js` accepts metres per second for a custom
   transporter, stores it, round-trips it and prints it. No simulation consumes it.
5. **The arrival band's coefficient of variation has two values.** `1.3 - 0.7` is
   `0.6000000000000001` in IEEE-754, so computing the coefficient from the band's endpoints gives
   `0.17320508075688776` while the exact-variance route gives `0.17320508075688773`. Only the second
   squares back to exactly `0.03` and satisfies `sqrt(3) x cv === 0.3`. Both are registered, labelled,
   because which one a caller uses decides whether its arithmetic round-trips.

**Nothing reads the registry, and that is deliberate.** Every value is a copy of a literal that is
still where it was, so every run, run id, fixture and pinned digest is exactly what it was — asserted,
not assumed: the harness re-records the hand floor's 300-tick export and compares it byte for byte
with fixture A, and greps every model file for a reference to the registry and finds none. Adoption at
the call sites is a later, separately gated step, one site at a time.

**Two sites will keep their literal, and the reason is floating point rather than taste.** The stage
model multiplies a capacity by a precomputed reciprocal, and `cap x (1/tph)` is not `cap / tph`. And
re-expressing the arrival ripple as `1 + (2u - 1) x 0.3` instead of `0.7 + 0.6u` is equal by algebra
and unequal in floating point for **60 % of the generator's outputs, first at the very first draw** —
so a declared coefficient of variation will be a different run with a different id, never the same run
re-expressed. Both are named in the module and pinned in the harness.

**The source scan earned its keep on its first run.** `unregistered(source)` scans a module's text for
rate-shaped declarations and reports those the registry lacks. It found a manufacturing source's emit
rate that had been missed by hand. It reads names, so a rate called something unexpected can hide from
it, and the two literals it reports that are correctly not entries — an accumulator initialised to
zero, and a labour cost in euros — are pinned with the reason.

`verify_units.js` (22 checks), the self-test `units-registry-agrees-with-the-live-modules` (198/198),
`docs/UNITS_AND_RATES.md` generated by `tools/units_page.mjs` with a `--check` freshness gate, cache
`wt-v148`.
