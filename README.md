...
- **Verified at the current commit (2026-09-21)** — `node test/run-all.mjs`: **61 headless harnesses** green; `python -m pytest test`: **62 Python tests** green; `index.html?selftest=1` in headless Chromium: **WT-SELFTEST PASS 168/168**; service-worker cache `wt-v111`. These checks do not establish plant safety. See [placement enforcement gaps](docs/PLACEMENT_ENFORCEMENT_AUDIT.md). The PWA caches local assets; network-disconnected behavior was not revalidated in this increment.
