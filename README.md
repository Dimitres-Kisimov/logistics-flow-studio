...
- **Verified at the current commit (2026-09-22)** — `node test/run-all.mjs`: **68 headless harnesses** green; `python -m pytest test`: **84 Python tests** green; `index.html?selftest=1` in headless Chromium: **WT-SELFTEST PASS 176/176**; service-worker cache `wt-v118`. These checks do not establish plant safety. See [placement enforcement gaps](docs/PLACEMENT_ENFORCEMENT_AUDIT.md). The PWA caches local assets; network-disconnected behavior was not revalidated in this increment.
