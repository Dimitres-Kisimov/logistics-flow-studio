...
- **Real board data.** The ECT and caliper per profile are synthetic teaching values; a verdict on a real stack needs the supplier's board specification, the measured humidity and the time under load. The McKee formula is applied outside its published range only with a flag.
