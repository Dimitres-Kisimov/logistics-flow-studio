...
**The control tower's audit (v3.56).** The app keeps the tower's decisions across runs and hands the log to `ledger.create(plan, { control })`; the export carries `control: [{ seq, tick, rule, proposal_id, status: accepted | declined | snoozed, lever: { kind: picker | kb, key, value }, evidence, from_run }]` only when a decision exists. The run id does not include the audit (it is not an input); an accepted lever is an input of the run that came after. `WT.control.controlRows(export)` aggregates the rows per rule (the `v_control` twin). Nothing in the audit names a person.
...
| `v_control` | per rule (v3.56): the proposals a person decided, accepted / declined / snoozed, the first tick - reconciled against `WT.control.controlRows`; no row without a decision |
