...
*The contract between the simulator, the SQLite tool and the viewer. Written 2026-09-22 for v3.32–v3.46; the tracking twins added 2026-09-23 for v3.53.*
...
**Tracking twins (v3.53).** Every handling event has exactly one EPCIS-shaped twin, *derived* from this export by `tracking.js` (`WT.tracking.fromLedger(exp)`, or incrementally `observe(track, rec)` during a run) and, at import, by the Python twin of the same mapping in `tools/run_ledger.py` (`track_events`). The twin document, schema `factory-tracking-events/v1`, carries `{ schema, honesty, notes, vocabulary, run: { id, scenario, seed, hash, ticks, minutes_per_tick }, events }`; an event is `{ eventID: "urn:wt:evt:<EVT id>", type: ObjectEvent | AggregationEvent, action: ADD | OBSERVE | DELETE, eventTime: null, "wt:tick", "wt:minute", "wt:kind", "wt:op", "wt:hu_id", "wt:version", epcList + quantityList | parentID + childQuantityList, bizStep, disposition, readPoint: { id: sgln URN, "wt:element" } | { id: null, "wt:zone" }, bizLocation: { id: "urn:wt:zone:<stage>" } | null, bizTransactionList: [{ type: po | rma, bizTransaction: <order id> }, { type: "wt:order_ref", … }?], "wt:error": null, "wt:delivery": null }`. GS1 EPCIS 2.0 / CBV 2.0 identifiers are used as vocabulary; no conformance is claimed (no wall clock; EPC URIs on GS1's documentation prefix). The mapping table is in [DIGITAL_TWIN_DEEP_DIVE.md](DIGITAL_TWIN_DEEP_DIVE.md) chapter 6; `verify_tracking.js` proves one twin per event, the terminals, the aggregation quantities, dwell == spans, and that a run with the tracker attached is byte-identical to one without. The committed `test/fixtures/run-ledger.tracking.json` is `fromLedger` of fixture A; `run-ledger.tracking.views.json` its dwell and disposition rows. Events are keyed to a handling unit, an element and a business step - never to a person.

**Recorded events (v3.58).** The same table takes a recorded EPCIS 2.0 capture document: `python tools/epcis_import.py import <document.json> --database run.sqlite` (the Python twin of `tracking.js fromEpcis`) writes one `run` row (scenario `epcis-import`, `minutes_per_tick 1`) and one `tracking_event` row per mapped event with `source = 'imported'`, `handling_event_id` NULL, `hu_id` the document's own object identifier, `wt_kind` recorded, `wt_op` NULL; a derived twin carries `source = 'derived'`. `tracking_event` was rebuilt for it (the `source` column with its CHECK, the ledger-only constraints dropped; an older database is rebuilt once at open) and `v_epcis_events` joins `hu` on the left. `v_bizstep_dwell`, `v_unit_history` and `v_tracking_gaps` work unchanged over both sources. The rule and the refusals: [EPCIS_IMPORT.md](EPCIS_IMPORT.md).

**Human error (v3.54).** When the flow ran with the error what-if (`flowsim` `opts.errors`, the planner's *Human error → Declared*), `run.errors = { kind: "human-error", kinds: [{ kind, ops, share, effective, disposition, rework, source }], psf, multiplier, latent, cap, honesty }` and every unit on an error branch carries `error_kind`, `error_op`, `error_outcome` (`rework` | `scrap`) and `error_latent` (the levers above 1). The route id names the branch (`piece-pick:mis-pick@piece-pick`); a rework lists the operation, its verification (`verify-pick` / `verify-put`) and the operation again, so a unit's events can carry the same operation twice - the quantity at an event is resolved by the op's index in the route. `WT.ledger.qualityByStep(export)` gives ISO 22400-2's names per operation (units through, errors, reworked, scrapped for damage, first pass yield, rework ratio, scrap ratio); `stats(rec).quality` carries them only when the what-if ran. Shares are teaching values (HEART / SPAR-H anchors) attributed to a step and a latent condition, never to a person.

**Delivery windows (v3.55).** When the flow ran with the delivery what-if (`flowsim` `opts.inbound` / `opts.outbound`, the planner's *Delivery → Dock and carrier windows*), `run.inbound = { kind: "dock-windows", periodTicks, openTicks, lateness[], mode, scaleTicksPerDay, source, honesty }` and `run.outbound = { kind: "carrier-windows", periodTicks, promisedLeadTicks, transit[], mode, scaleTicksPerDay, source, honesty }`; every unit carries `trailer` (the trailer it came off), `due_tick` (spawn + the promised lead), `transit_ticks` (by sequence) and, once delivered, `customer_tick`, `on_time_shipped`, `on_time`; the export carries `inbound: [{ trailer, scheduled_tick, arrival_tick, late_ticks }]`, the trailers the door logged. `WT.ledger.serviceOf(export)` gives orders, delivered orders, OTIF orders and share (every unit of the order delivered by its due tick), the shipped-on-time share and the mean transit; `WT.ledger.inboundRows(export)` the trailers with their units. The lateness list is the Weyl sequence through the quantiles of the USAID SCMS delivery history ([SCMS_DELIVERY.md](SCMS_DELIVERY.md)), scaled by a teaching parameter - never a random draw.

**The control tower's audit (v3.56).** The app keeps the tower's decisions across runs and hands the log to `ledger.create(plan, { control })`; the export carries `control: [{ seq, tick, rule, proposal_id, status: accepted | declined | snoozed | reverted (v3.57), lever: { kind: picker | kb, key, value, from? }, evidence, from_run }]` only when a decision exists. A `reverted` row (v3.57) names the proposal it undoes in `evidence.reverts`, its `lever.value` is the value put back and `lever.from` the one taken away; a database created before v3.57 rebuilds `control_event` once at open because SQLite cannot alter the status CHECK. The run id does not include the audit (it is not an input); an accepted lever is an input of the run that came after. `WT.control.controlRows(export)` aggregates the rows per rule (the `v_control` twin). Nothing in the audit names a person.
...
| `v_replication_groups`, `v_replication_cycle_by_type`, `v_replication_cost_by_type`, `v_replication_summary` | runs that share scenario, order mix, ticks, policy and (v3.55) the error and delivery levers but differ in seed, as a group: n and the seeds; per order type (cycle ticks, cost) and per run total (units, delivered, total_eur): n, mean, the sample standard deviation (two-pass), a Student-t two-sided 95 % half-width t(n−1) × s / √n from the seeded `t_critical` table (df 1–30; beyond, 1.960), min, max — not keyed by run (v3.46, see §8) |
| `v_quality_by_step` | per operation (v3.54): units through, errors, reworked, scrapped for damage, first pass yield, rework ratio, scrap ratio - ISO 22400-2's names; errors 0 and first pass yield 1 on a run without the what-if; reconciled against `WT.ledger.qualityByStep` |
| `v_control` | per rule (v3.56): the proposals a person decided, accepted / declined / snoozed / reverted (v3.57), the first tick - reconciled against `WT.control.controlRows`; no row without a decision |
| `v_otif` | per run with carrier windows (v3.55): orders, delivered orders, OTIF orders and share, shipped-on-time share, mean transit - reconciled against `WT.ledger.serviceOf`; no row without windows |
| `v_inbound` | per trailer the door logged (v3.55): scheduled, arrival and late ticks, the units it brought (`inbound_event` joined to `hu.trailer`) - reconciled against `WT.ledger.inboundRows` |
| `v_bizstep_dwell` | per business step of the tracking twins (v3.53): events, units, spans (events with a next event of the same unit), mean and maximum ticks to that next event, waiting ticks (queued twins) and their share - reconciled against `tracking.js dwellByBizStep` |
...
| `v_tracking_gaps` | a handling event without a tracking twin, or a twin whose business step or disposition is outside the CBV 2.0 vocabulary this app may emit (v3.53) |
...
**Detail views** (`views --all`): `v_wip_by_tick`, `v_spans`, `v_span_cost`, `v_cost_by_hu`, `v_dispatch_by_order`, and (v3.53) `v_epcis_events` - the twins joined to their units - and `v_unit_history` - one unit's steps with the ticks to its next event. Views are dropped and recreated on every open, so a database created by an older version always runs the current text.
...
|---|---|---|---|
| The run at a glance | identity, mix, totals, cost, the five invariants, the data-quality flags (no rates; stations at the floor rate; units in flight; no holding cost) | `RunLedger.glance`, `v_run_summary` | `verify_run_ledger_view.js` §5, the viewer self-test |
...
| What the planner asks (Control tower) | the decisions per rule and the raw audit (v3.56), with a note when nobody decided | `v_control` | `verify_control.js`, `test_run_ledger.py` (ControlTower) |
| What the planner asks (Delivery) | on time in full, shipped on time, the mean transit, the trailers at the door (v3.55), with a note when the run had no windows | `v_otif`, `v_inbound` | `verify_delivery.js`, `test_run_ledger.py` (Delivery) |
| What the planner asks (Quality by step) | first pass yield, rework ratio and scrap ratio per operation (v3.54), with a note naming the what-if or saying every step was perfect | `v_quality_by_step` | `verify_errors.js`, `test_run_ledger.py` (Quality) |
| Tracking | the run in the EPCIS vocabulary (v3.53): dwell per business step with the waiting separated, dispositions by step, the gap count; a unit's twins beside its trace | `RunLedger.views().bizstepDwell`, `v_bizstep_dwell`, `v_epcis_events`, `v_unit_history` | `verify_tracking.js`, `test_run_ledger.py` (Tracking) |
...
| The invariants | the five views that must return zero rows | `v_conservation_violations` … `v_terminal_violations` | `test_run_ledger.py` (deliberate corruption) |
...
node verify_tracking.js                      # the tracking twins: mapping, vocabulary, dwell == spans, the store, purity, export == observer (v3.53)
node verify_epcis_import.js                  # the return path: a recorded EPCIS 2.0 document by hand, refusals, the store, the Python twin's lists (v3.58)
python tools/epcis_import.py import test/fixtures/epcis-document.json --database work/run.sqlite   # recorded events beside the twins, source = imported (v3.58)
node verify_errors.js                        # human error: the quota by hand, the rework route, 20 routes, conservation, the quality KPIs, the twins' dispositions (v3.54)
node verify_delivery.js                      # delivery windows: the Weyl / quantile lateness by hand, the window openings, departures, OTIF by hand, the dataset twin (v3.55)
python tools/scms_delivery.py --offline-check   # the committed SCMS aggregates, their JS twin and Markdown agree (v3.55)
node verify_control.js                       # the control tower: four rules by hand, decisions and the audit, purity, the v_control twin (v3.56)
python tools/selftest_realtime.py            # the app self-test in real-time headless Chrome (Playwright) with &idb=1: the tracking store on IndexedDB, which the gate's virtual-time driver cannot reach (v3.53)
