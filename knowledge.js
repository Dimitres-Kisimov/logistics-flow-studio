...
(function () {
  ...
  const CATEGORIES = [
    ...
    { key: "people", label: "Learning and fatigue (shift what-if)",
      desc: "The two curves the learning and fatigue what-if reads: how much faster a step gets as its units accumulate (Wright's rate in Crawford's unit form), where that stops, how much slower it gets as the minutes since the last break add up, and the break rhythm. Declared per STEP and per SHIFT, never per person: the count is a station's, not a worker's, and nothing here may be keyed to one (BetrVG 87(1)6, GDPR Art. 88)." },
  ...
  const SPARH_NOTE = "Teaching value from SPAR-H's action worksheet (NUREG/CR-6883), the only public, documented set of generic performance-shaping multipliers there is - built for nuclear control rooms, not for a warehouse. It belongs to a process step, a workplace or a shift, never to a person; SPAR-H's eighth factor, fitness for duty, is refused for exactly that reason. A site replaces it with its own judgement of its own steps.";
  ...
  seed({ id: "hf.psf.signalToNoise", category: "human-factors", label: "Low signal-to-noise multiplier (label contrast, lighting, look-alike articles; 1 = none)", value: 1, unit: "x",
    source: "HEART error-producing condition: a low signal-to-noise ratio - maximum effect x10; the credit x0.5 from SPAR-H Ergonomics/HMI level 'Good' (NUREG/CR-6883, action worksheet).", note: HF_NOTE, editable: true, kind: "number", min: 0.5, max: 10 });
  seed({ id: "hf.psf.familiarity", category: "human-factors", label: "Unfamiliarity multiplier (training, a novel or infrequent task; 1 = none)", value: 1, unit: "x",
    source: "HEART error-producing condition: unfamiliarity with a situation which is potentially important but which only occurs infrequently or which is novel - maximum effect x17; the credit x0.5 from SPAR-H Experience/Training level 'High' (NUREG/CR-6883, action worksheet).", note: HF_NOTE, editable: true, kind: "number", min: 0.5, max: 17 });
  seed({ id: "hf.psf.stressors", category: "human-factors", label: "Workplace stressors multiplier (cold store, noise, cramped aisle, glare; 1 = nominal)", value: 1, unit: "x",
    source: "SPAR-H Stress/Stressors, action worksheet: High x2, Extreme x5 (NUREG/CR-6883). Declared in the method's own environmental reading - 'excessive heat, noise, poor ventilation' - and never as a claim about what a person feels.", note: SPARH_NOTE, editable: true, kind: "number", min: 1, max: 5 });
  seed({ id: "hf.psf.complexity", category: "human-factors", label: "Task complexity multiplier (mixed-SKU pallets, multi-touch picks, look-alike families; 1 = nominal)", value: 1, unit: "x",
    source: "SPAR-H Complexity, action worksheet: Moderately complex x2, Highly complex x5 (NUREG/CR-6883). 'How difficult the task is to perform in the given context' - a property of the step.", note: SPARH_NOTE, editable: true, kind: "number", min: 1, max: 5 });
  seed({ id: "hf.psf.procedures", category: "human-factors", label: "Procedures multiplier (a written instruction, a scan verification, a check; 1 = nominal)", value: 1, unit: "x",
    source: "SPAR-H Procedures, action worksheet: Available but poor x5, Incomplete x20, Not available x50 (NUREG/CR-6883). 'The existence and use of formal operating procedures for the tasks under consideration'.", note: SPARH_NOTE, editable: true, kind: "number", min: 1, max: 50 });
  seed({ id: "hf.psf.workProcesses", category: "human-factors", label: "Work-processes multiplier (shift handover, work planning, communication; 1 = nominal, 0.5 = good)", value: 1, unit: "x",
    source: "SPAR-H Work Processes, action worksheet: Poor x5, Nominal x1, Good x0.5 (NUREG/CR-6883). 'Work planning, communication and management support' - a property of the organisation and the shift, not of a person.", note: SPARH_NOTE, editable: true, kind: "number", min: 0.5, max: 5 });
  seed({ id: "hf.error.cap", category: "human-factors", label: "Cap on any effective error share", value: 0.5, unit: "share",
    source: "WarehouseTwin choice: the levers cannot push a step's share above one in two; when the cap binds the readout says so. It is a cruder limit than SPAR-H's adjustment factor, which applies first once three or more levers stand above nominal.", note: HF_NOTE, editable: true, kind: "number", min: 0.01, max: 1 });
  ...
  /* ---- v3.66 learning and fatigue: the two declared curves (people.js DEFAULTS hold the same numbers) ---- */
  const PE_NOTE = "A declared curve on a process step and a shift, never on a person - the learning count is the station's units in this run, the fatigue clock the minutes since the last break (BetrVG 87(1)6, GDPR Art. 88). Edit it and the next run uses it.";
  seed({ id: "people.learning.rate", category: "people", label: "Learning rate (the n-th unit takes n^log2(rate) of the first)", value: 0.95, unit: "share", source: "Wright (1936) as implemented in Crawford's unit-time form; rates of 0.80-0.95 are commonly quoted for repetitive manual work - a teaching value, not a measurement.", note: PE_NOTE, editable: true, kind: "number", min: 0.5, max: 1 });
  seed({ id: "people.learning.floor", category: "people", label: "Learning floor: the service time never falls below this share of nominal", value: 0.7, unit: "share", source: "WarehouseTwin choice: a bench stops improving somewhere, and an unbounded curve would drive the service time to zero.", note: PE_NOTE, editable: true, kind: "number", min: 0.1, max: 1 });
  seed({ id: "people.fatigue.maxUplift", category: "people", label: "Fatigue: the most the service time rises before a break", value: 0.15, unit: "share", source: "Work-study practice (ILO, Introduction to Work Study) adds a rest allowance of a few per cent of basic time plus variable allowances; Grosse et al. (IJPR 2015) name fatigue as omitted from picking models. A teaching value in that spirit.", note: PE_NOTE, editable: true, kind: "number", min: 0, max: 2 });
  seed({ id: "people.fatigue.toPeakMinutes", category: "people", label: "Fatigue: minutes since the last break at which the uplift is fully reached", value: 240, unit: "minutes", source: "WarehouseTwin choice: four hours to the peak - a teaching value.", note: PE_NOTE, editable: true, kind: "number", min: 1, max: 1440 });
  seed({ id: "people.fatigue.breakEveryMinutes", category: "people", label: "Break rhythm: minutes of work between breaks", value: 120, unit: "minutes", source: "WarehouseTwin choice: a break every two hours, assumed STAGGERED - it resets the fatigue clock and does not stop the bench.", note: PE_NOTE, editable: true, kind: "number", min: 1, max: 1440 });
  seed({ id: "people.fatigue.breakMinutes", category: "people", label: "Break length (the fatigue clock reads zero through it)", value: 15, unit: "minutes", source: "WarehouseTwin choice: fifteen minutes - a teaching value.", note: PE_NOTE, editable: true, kind: "number", min: 0, max: 240 });
