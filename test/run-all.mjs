...
const HARNESSES = [
  ...
  { name: "A real order book behind the demo: 1 067 371 rows of Online Retail II (CC BY 4.0) reduced by a stated rule, the app's own parsers reading 2 000 real articles and one real trading day (132 orders, 3 533 lines) with zero errors, the measured ABC curve, a cancellation reported as a cancellation and never as a return (verify_online_retail.js)", args: ["verify_online_retail.js"] },
  { name: "One place where a speed is declared: 38 rates and speeds registered with their unit and provenance and asserted against the live modules, the conversions by hand, the app's two travel speeds shown to differ by 205.71x, one tick meaning a minute in one model and a quarter hour in another, and the arrival band's coefficient of variation shown to have two values that differ in the last digit (verify_units.js)", args: ["verify_units.js"] },
  { name: "The route as a declared graph: every archetype and every error branch walks to exactly the list v3.67 built by concatenation (9 + 14 pinned literally), the rework edge points backward with its visit bound, the graph is total, and docs/ROUTE_GRAPH.md is generated fresh from the code (verify_routegraph.js)", args: ["verify_routegraph.js"] },
  { name: "The rest of SPAR-H: seven performance-shaping levers with the action worksheet's published levels, fitness for duty refused in the model, the adjustment factor on the standard's own worked example (composite 400 on 0.01 -> 0.801603), the credits that declare a design good (verify_psf.js)", args: ["verify_psf.js"] },
