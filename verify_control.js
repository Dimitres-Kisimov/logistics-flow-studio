 ...
 *      the runner, sw.js at wt-v135, README / CHANGELOG / the deep dive's
...
(function () {
  ...
  const two = { control: [{ seq: 1, tick: 130, rule: "queue-congestion", status: "declined" }, { seq: 2, tick: 250, rule: "queue-congestion", status: "snoozed" }, { seq: 3, tick: 460, rule: "inbound-late", status: "accepted" }] };
  check("5c. controlRows (the v_control twin) by hand on a three-row audit: queue-congestion 2 proposals (1 declined, 1 snoozed, first 130); inbound-late 1 accepted (first 460); sorted by rule",
    JSON.stringify(C.controlRows(two)) === JSON.stringify([{ rule: "inbound-late", proposals: 1, accepted: 1, declined: 0, snoozed: 0, first_tick: 460 }, { rule: "queue-congestion", proposals: 2, accepted: 0, declined: 1, snoozed: 1, first_tick: 130 }]) && C.controlRows({}).length === 0);
})();
...
(function () {
  ...
  check("6e. the viewer: the Control tower block with its note, the glance card; both self-tests; run-all; sw.js precaches control.js at wt-v135 (previously wt-v134)",
    ...
    /Object\.keys\(R\.SQL\)\.length === 37/.test(vst) && /verify_control\.js/.test(runall) && /"\.\/control\.js"/.test(sw) && /CACHE_VERSION\s*=\s*"wt-v135"/.test(sw) && /Previously wt-v134/.test(sw));
