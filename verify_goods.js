 ...
 *      workers.js + precached at wt-v126, app.js draws it in BOTH render
...
(() => {
  ...
  const inSw = /["']\.\/goods\.js["']/.test(SW_SRC) && /CACHE_VERSION\s*=\s*"wt-v126"/.test(SW_SRC);
  ...
  check("shipped wiring: index.html loads goods.js AFTER workers.js and before app.js, sw.js precaches it at wt-v126, app.js draws the goods + the truck loads in BOTH render paths through projPx (LOD-gated with a drawing budget, reduced-motion-safe, rack stock handed to both the glyph and the 2.5D forms), and the self-test + runner cover it",
