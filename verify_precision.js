 ...
 *      string, sw.js precaching C at wt-v132, the reconcile command and
...
(function () {
  ...
  check("4c. sw.js precaches fixture C at wt-v132 (previously wt-v131)", /"\.\/test\/fixtures\/run-ledger-c\.json"/.test(sw) && /CACHE_VERSION\s*=\s*"wt-v132"/.test(sw) && /Previously wt-v131/.test(sw));
  check("4d. tools/run_ledger.py has the reconcile command over 16 views with the two tolerances (v_bizstep_dwell since v3.53)", /def reconcile\(/.test(py) && /"reconcile"/.test(py) && (py.match(/"v_[a-z_]+": \(/g) || []).length === 16 && /--tolerance-raw/.test(py));
