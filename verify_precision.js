 ...
 *      string, sw.js precaching C at wt-v123, the reconcile command and
...
(function () {
  ...
  check("4b. the fixture script documents [a|b|c|all] and the reconcile sub-command, and builds a and b before loading wms.js",
    /\[a\|b\|c\|all\]/.test(mk) && /reconcile <dir>/.test(mk) && /build a and b before c/.test(mk) && /MODULES_SCENARIO/.test(env) && /wms\.js/.test(env));
  check("4c. sw.js precaches fixture C at wt-v123 (previously wt-v122)", /"\.\/test\/fixtures\/run-ledger-c\.json"/.test(sw) && /CACHE_VERSION\s*=\s*"wt-v123"/.test(sw) && /Previously wt-v122/.test(sw));
  check("4d. tools/run_ledger.py has the reconcile command over 13 views with the two tolerances", /def reconcile\(/.test(py) && /"reconcile"/.test(py) && (py.match(/"v_[a-z_]+": \(/g) || []).length === 13 && /--tolerance-raw/.test(py));
