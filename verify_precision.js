 ...
 *      string, sw.js precaching C at wt-v124, the reconcile command and
...
(function () {
  ...
  check("4c. sw.js precaches fixture C at wt-v124 (previously wt-v123)", /"\.\/test\/fixtures\/run-ledger-c\.json"/.test(sw) && /CACHE_VERSION\s*=\s*"wt-v124"/.test(sw) && /Previously wt-v123/.test(sw));
  check("4d. tools/run_ledger.py has the reconcile command over 15 views with the two tolerances", /def reconcile\(/.test(py) && /"reconcile"/.test(py) && (py.match(/"v_[a-z_]+": \(/g) || []).length === 15 && /--tolerance-raw/.test(py));
