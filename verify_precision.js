 ...
 *      string, sw.js precaching C at wt-v133, the reconcile command and
...
(function () {
  ...
  check("4c. sw.js precaches fixture C at wt-v133 (previously wt-v132)", /"\.\/test\/fixtures\/run-ledger-c\.json"/.test(sw) && /CACHE_VERSION\s*=\s*"wt-v133"/.test(sw) && /Previously wt-v132/.test(sw));
  check("4d. tools/run_ledger.py has the reconcile command over 17 views with the two tolerances (v_bizstep_dwell since v3.53, v_quality_by_step since v3.54)", /def reconcile\(/.test(py) && /"reconcile"/.test(py) && (py.match(/"v_[a-z_]+": \(/g) || []).length === 17 && /--tolerance-raw/.test(py));
