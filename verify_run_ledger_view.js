 ...
 *      at wt-v144; the planner has the hand-over button; the offline guard
...
(function () {
  ...
  check("4b. sw.js precaches the page, its script, stylesheet and the recorded example at wt-v144 (previously wt-v143)",
    /"\.\/run-ledger\.html"/.test(sw) && /"\.\/run-ledger\.js"/.test(sw) && /"\.\/run-ledger\.css"/.test(sw) && /"\.\/test\/fixtures\/run-ledger\.json"/.test(sw) &&
    /CACHE_VERSION\s*=\s*"wt-v144"/.test(sw) && /Previously wt-v143/.test(sw));
