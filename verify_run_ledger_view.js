 ...
 *      at wt-v147; the planner has the hand-over button; the offline guard
...
(function () {
  ...
  check("4b. sw.js precaches the page, its script, stylesheet and the recorded example at wt-v147 (previously wt-v146)",
    /"\.\/run-ledger\.html"/.test(sw) && /"\.\/run-ledger\.js"/.test(sw) && /"\.\/run-ledger\.css"/.test(sw) && /"\.\/test\/fixtures\/run-ledger\.json"/.test(sw) &&
    /CACHE_VERSION\s*=\s*"wt-v147"/.test(sw) && /Previously wt-v146/.test(sw));
