 ...
 *      at wt-v141; the planner has the hand-over button; the offline guard
...
(function () {
  ...
  check("4b. sw.js precaches the page, its script, stylesheet and the recorded example at wt-v141 (previously wt-v140)",
    /"\.\/run-ledger\.html"/.test(sw) && /"\.\/run-ledger\.js"/.test(sw) && /"\.\/run-ledger\.css"/.test(sw) && /"\.\/test\/fixtures\/run-ledger\.json"/.test(sw) &&
    /CACHE_VERSION\s*=\s*"wt-v141"/.test(sw) && /Previously wt-v140/.test(sw));
