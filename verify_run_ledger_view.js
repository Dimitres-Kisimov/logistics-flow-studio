 ...
 *      at wt-v133; the planner has the hand-over button; the offline guard
...
(function () {
  ...
  check("4b. sw.js precaches the page, its script, stylesheet and the recorded example at wt-v133 (previously wt-v132)",
    /"\.\/run-ledger\.html"/.test(sw) && /"\.\/run-ledger\.js"/.test(sw) && /"\.\/run-ledger\.css"/.test(sw) && /"\.\/test\/fixtures\/run-ledger\.json"/.test(sw) &&
    /CACHE_VERSION\s*=\s*"wt-v133"/.test(sw) && /Previously wt-v132/.test(sw));
...
(function () {
  const SQL_NAMES = ["v_cycle_time_by_type", "v_touches_by_type", "v_station_wait", "v_wip_by_tick", "v_quantities_by_op", "v_dispatch", "v_run_summary", "v_flow_links",
    ...
    "v_epcis_events", "v_unit_history", "v_bizstep_dwell", "v_tracking_gaps", "v_quality_by_step"]; // v3.53, v3.54
