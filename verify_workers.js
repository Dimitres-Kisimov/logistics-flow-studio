 ...
 *      the bumped wt-v112 cache, app.js draws the workforce in BOTH render
...
(() => {
  ...
  const inSw = /["']\.\/workers\.js["']/.test(SW_SRC) && /CACHE_VERSION\s*=\s*"wt-v112"/.test(SW_SRC);
  ...
  check("shipped wiring: index.html loads workers.js before app.js, sw.js precaches it at wt-v112, app.js draws the workforce in BOTH render paths through projPx (LOD-gated, reduced-motion-safe, posed from the sim tick), and the self-test + runner cover it",
